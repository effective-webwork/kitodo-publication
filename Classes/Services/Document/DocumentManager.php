...
class DocumentManager
{
    ...
    )
    {
        ...
        if ($document->isSuggestion()) {
            ...
            var_dump($document->getPrimaryUrn());
            die();
    ...
    protected function initPrimaryUrnIfEligible($document) {
        ...
        if (empty($primaryUrn)) {
            ...
            $mandatoryPrimaryUrnCollections = $clientConfigurationManager->getMandatoryPrimaryUrnCollections();

            $common = array_intersect(
                array_map('strtolower', $mandatoryPrimaryUrnCollections),
